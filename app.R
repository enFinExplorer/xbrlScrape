  ...
  shinyApp(
    ui = bulmaPage(
      ...
      bulmaHero(
        ...
        bulmaHeroBody(
          bulmaContainer(
            bulmaTitle("Energy Financial Explorer"),
            bulmaSubtitle("XBRL Scraping Attempts")
          ...
          bulmaTileAncestor(
            bulmaTileParent(
              ...
              bulmaTileChild(
                ...
                bulmaSelectInput('operator', 'Select Company',
                                 choices = unique(opLink$operator)),
                bsButton('loadFilings', 'Load Filings', icon = icon('table'), size = 'default', style = 'default'),
                DT::dataTableOutput("filingList"),
                #actionButton('loadFilings', 'Load Filings', icon = icon('table')),
            ...
            bulmaTileParent(
              vertical = TRUE,
              bulmaTileChild(
                bulmaTitle("Stock Performance"),
                bulmaDateInput('start_date', value = as.POSIXct('2019-01-01', format = '%Y-%m-%d'), max = Sys.Date(), lang = 'en'),
                echarts4rOutput('hcplot'),
                color = NULL
              ),
              bulmaTileChild(
                bulmaTitle("Period Selection"),
                selectizeInput('Filing', 'Filing', choices = ''),
                bsButton('calc', 'Load Tables', icon = icon('table'), size = 'default', style = 'default'),
                color = "info"
              ),
              bulmaTileChild(
                bulmaTitle("Balance Sheet"),
               DT::dataTableOutput('bs'),
                color =  NULL
              ),
              bulmaTileChild(
                bulmaTitle("Income Statement"),
                DT::dataTableOutput('is'),
                color = NULL
              ),
              bulmaTileChild(
                bulmaTitle("Cash Flow Statement"),
                DT::dataTableOutput('cf'),
                color = NULL
              ),
              bulmaTileChild(
                bulmaTitle('Other Tables'),
                selectizeInput('role', 'Avalable Tables', choices = ''),
    ...
    server = function(input, output, session) {
      ...
      observe({
        if(is.null(values$xbrl.vars)){
          ...
          updateSelectizeInput(session, 'role', choices = unique(test1$description))
        }
      })
